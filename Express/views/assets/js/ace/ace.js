		...
		if($.fn.ace_ajax) {
		   $('[data-ajax-content=true]').ace_ajax({
			 ...
			 'content_url': function(hash) {
				...
				//for example in Ace HTML demo version we convert /ajax/index.html#page/gallery to > /ajax/gallery.html and load it
				if(path.match(/index\.html?/))
					return path.replace(/index\.html?/, '/ace/'+hash.replace(/^page\//, '')+'.html') ;

				//for example in Ace PHP demo version we convert "ajax.php#page/dashboard" to "ajax.php?page=dashboard" and load it
				return path + "?" + hash.replace(/\//, "=");
			  },
			  
			  'default_url': 'page/default'//default hash
		   })
		}
	}
